...
function abstract_call_gf_by_type(interp::AbstractInterpreter, @nospecialize(f), argtypes::Vector{Any}, @nospecialize(atype), sv::InferenceState,
    ...
    if splitunions
        ...
        for sig_n in splitsigs
            xapplicable = _methods_by_ftype(sig_n, max_methods, get_world_counter(interp), min_valid, max_valid)
            xapplicable === false && return Any
            append!(applicable, xapplicable)
        end
    else
        applicable = _methods_by_ftype(atype, max_methods, get_world_counter(interp), min_valid, max_valid)
        if applicable === false
...
function abstract_iteration(interp, @nospecialize(itft), @nospecialize(itertype), vtypes::VarTable, sv::InferenceState)
